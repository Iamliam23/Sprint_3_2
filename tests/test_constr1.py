...
class TestConstructor:
    def test_stellarburgers_check_span_nbr_of_element_is_ok(self, driver, get_the_page):
    ...
    def test_stellarburgers_scroll_check_nbr_of_elements_is_ok(self, driver, get_the_page):
        ingredients_list = WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located((Locators.MENU_CONTAINER)))
    ...
    def test_stellarburgers_check_nbr_of_elements_before_after_is_ok(self, driver, get_the_page):
        ...
        fillings = WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located((Locators.FILLINGS_SPAN_2)))
        fillings.click()
        element_1 = WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located((Locators.FILLINGS_INGR_2)))
        action = ActionChains(driver)
        action.click_and_hold(element_1)
        action.move_by_offset(5, -5)
        action.release()
        action.perform()
        span_elements = WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located((Locators.DESCENDANTS_UL)))
